...
namespace Scripting.CSharp
{
        ...
        : ScriptEarlyBound
    {
        ...
        protected override void OnExecute(IWorker worker)
        {
            ...
            GetIOList_Light(worker, device, IO_List);//Gets a list of IO devices.
            GeneratePlcProject(devices, worker);
            AddAutoGeneratedCode(devices, worker); // This use to be after the "add librarys" method. But found it locks up during compile so, moved it here.
            ...
            GetIOList(worker, device, IO_List);//Reads the IO list and search through it for inputs and outputs. 
        ...
        private void GeneratePlcProject(ITcSmTreeItem devices, IWorker worker)
        {
            //At the moment this isn't enough. I think I need to just have it import a template pLC project.
            ITcSmTreeItem plcConfig = systemManager.LookupTreeItem("TIPC");
            if(plcConfig.ChildCount == 0)
            { //            ITcSmTreeItem plcProject = PlcDevice.Child[1];
                string plcProjectName = "PLC1";
                string vsXaePlcEmptyTemplateName = "Empty PLC Template.plcproj";
...
"\t//Code added by the programmer here will be deleted.\n");
        ...
        private void AddManualLinks(IWorker worker)
        {
            ...
            foreach (ManualLinks Link in CSV_Reader.Links)
            {
                ...
                try
                {
                    ...
                    ITcSmTreeItem Temp = systemManager.LookupTreeItem(Link1);

                    XmlDocument xmlItem = new XmlDocument();
                    xmlItem.LoadXml(Temp.ProduceXml());
                    XmlNode LinkedWith = xmlItem.SelectSingleNode("TreeItem/VarDef/LinkedWith"); //Gets the I/O Type. Example EP1111 or EK1100
                    if (LinkedWith != null && LinkedWith.InnerText.Length > 0)// Want to ignore the link if its already linked. It appears we can get double links.
                    {
                        ;//Possibly in the future, we may want to unlink the tag.
                    }
                    else
        ...
        private void ImportEIPDevice(IWorker worker, ITcSmTreeItem devices, List<ReferenceCode.IO_Object> IoList, string EL6652Path)
        {
            ...
            if (CSV_Reader.PackBitsIntoBytes) //Simple flag that tells the program that we need to try to concern variable names
            {

                bool BitTesting = false;
                string terminalName = "";
                int BitCount = 0;
                int Pos = 0;
                string[] PathParrent = {};
                ...
                for (int i = 0; i < IoList.Count; i++)
                {
                    if (IoList[i].bitsize == 1)
                    {
                        if (BitTesting)
                        {
                            ...
                            string[] PathCurrent = { };
                            if (IoList[i].PathName.LastIndexOf(IoList[i].TerminalName) + IoList[i].TerminalName.Length + 1 < IoList[i].PathName.Length)
                                PathCurrent = IoList[i].PathName.Substring(IoList[i].PathName.LastIndexOf(IoList[i].TerminalName) + IoList[i].TerminalName.Length + 1).Split('^');
                            if (terminalName == IoList[i].TerminalName && PathCurrent.Count() == PathParrent.Count())//FrontPath4 == FrontPath2 && IoList[i].IsInput == IoList[i-1].IsInput)
                            {
                                BitCount++;
                            }
                            else
                            ...
                            if (i + 1 < IoList.Count)
                            {
                                ...
                                string[] PathNext = { };
                                if (IoList[i + 1].PathName.LastIndexOf(IoList[i + 1].TerminalName) + IoList[i + 1].TerminalName.Length + 1 < IoList[i + 1].PathName.Length)
                                    PathNext = IoList[i+1].PathName.Substring(IoList[i+1].PathName.LastIndexOf(IoList[i+1].TerminalName) + IoList[i+1].TerminalName.Length + 1).Split('^');
                                if (terminalName != IoList[i + 1].TerminalName || IoList[i].IsInput != IoList[i + 1].IsInput || IoList[i].bitsize != 1 || PathCurrent.Count() != PathNext.Count()) //FrontPath6 != FrontPath4 || IoList[i].IsInput != IoList[i + 1].IsInput)
                                {
                                    if (BitCount > 6 && BitCount < 17)
                                    {
                                        StartPack = true;
                                    }
                                    else
                            ...
                            if (StartPack)
                            {
                                StartPack = false;//Pack
                                BitTesting = false;
                                IoList[Pos].Description = "";
                                string newName = IoList[Pos].TerminalName + "_";
                                for (int z = 0; z < PathParrent.Count()-1; z++) {
                                    newName = newName + PathParrent[z] + "_";
                                }
                                newName = newName.Replace(" ", "_");
                                newName = newName.Trim('_');
                                //newName = newName.TrimEnd("Chl");
                               //newName = newName.TrimEnd("Channel");

                                if (PathParrent.Count() == 2) //Special case
                                {
                                ...
                                newName = newName.Trim('_');
                                if (PathParrent.Last().ToLower() == "Output".ToLower() || PathParrent.Last().ToLower() == "Input".ToLower())
                                {
                                    newName = newName+ "_" + PathParrent.Last() + "s";
                                }



                                    // if (IoList[Pos].IsInput)
                                    //    newName = IoList[Pos].PathName.Substring(IoList[Pos].PathName.LastIndexOf(IoList[Pos].TerminalName)).Replace('^', '_');//IoList[Pos].EIPName = IoList[i].TerminalName + "_Inputs";
                                    //else
                                    IoList[Pos].EIPName = newName; //IoList[i].TerminalName + "_Outputs";
                                for (int x = Pos; x < Pos + BitCount; x++)
                                {
                                    string NewDescription = "";
                                    if (IoList[x].PathName.LastIndexOf(IoList[x].TerminalName) + IoList[x].TerminalName.Length + 1 < IoList[x].PathName.Length)
                                        NewDescription = IoList[x].PathName.Substring(IoList[x].PathName.LastIndexOf(IoList[x].TerminalName) + IoList[x].TerminalName.Length + 1).Replace('^','_');
                                    
                                    
                                    IoList[x].PackBitsParrent = Pos;
                                    IoList[x].PackBitsOffset = x - Pos;
                                    IoList[Pos].Description = IoList[Pos].Description + "." + IoList[x].PackBitsOffset.ToString() + " " + NewDescription + Environment.NewLine;
                                }
                            }
                        }
                        else
                        {
                            terminalName = IoList[i].TerminalName;
                            if (IoList[i].PathName.LastIndexOf(IoList[i].TerminalName) + IoList[i].TerminalName.Length + 1 < IoList[i].PathName.Length)
                                PathParrent = IoList[i].PathName.Substring(IoList[i].PathName.LastIndexOf(IoList[i].TerminalName) + IoList[i].TerminalName.Length + 1).Split('^');
                            BitCount = 1;
                            BitTesting = true;
                            Pos = i;
                        }
            ...
            if (CSV_Reader.WcStatesPackIntoBits) //Simple flag that tells the program that we need to try to conserve variable names
            {
                int WcStateCounter = 0;
                int packOffset = 0;
                int PackParrent = 0;

                for (int i = 0; i < IoList.Count; i++)
                {
                    if (IoList[i].bitsize == 1 && IoList[i].ItemSubType == 3 && IoList[i].TagName.Contains("WcState"))
                    {
                        WcStateCounter++;
                        if (packOffset == 0)
                        {
                            PackParrent = i;
                            IoList[i].Description = "";
                            IoList[i].EIPName = IoList[i].PathName.Substring(IoList[i].PathName.LastIndexOf('^')+1) + WcStateCounter.ToString();
                            IoList[i].EIPName = IoList[i].EIPName.Replace(" ", "_");

                        }
                        IoList[PackParrent].Description = IoList[PackParrent].Description + "." + packOffset.ToString()+ " " + IoList[i].TagName + Environment.NewLine;
                        IoList[i].PackBitsParrent = PackParrent;
                        IoList[i].PackBitsOffset = packOffset;

                        packOffset++;
                        if (packOffset == 16)
                        {
                            IoList[PackParrent].EIPName = IoList[PackParrent].EIPName + "_" + WcStateCounter.ToString();
                            packOffset = 0;
                        }
            ...
            for (int x = 0; x < 4; x++)
            {
                for (int i = 0; i < IoList.Count; i++)
                {
                    ...
                    string TempTrim = tagName.Trim('_', ' ');
                    if (TempTrim != tagName) {
                        worker.ProgressStatus = "TagName: " + tagName + " trimming _ and white spaces so RS logics 5000 can import correctly";
                        ...
                        if (IoList[i].IsInput)
                    {
                        ...
                        if (IoList[i].PackBitsOffset == 0)
                        {
                            TotalInputs++;
                            IoList[i].item = Outputs.CreateChild(IoList[i].EIPName, 5, "", 1); //Use type 5 for a word
                            item = IoList[i].item;
                        ...
                        else if (IoList[i].PackBitsOffset > 0)
                        {
                            item = IoList[IoList[i].PackBitsParrent].item;//Outputs.Child[Outputs.ChildCount];
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (IoList[i].PackBitsOffset == 0)
                        {
                            ...
                            IoList[i].item = Inputs.CreateChild(IoList[i].EIPName, 5, "", 1); //Use type 5 for a word
                            item = IoList[i].item;
                            xmlDoc.LoadXml(item.ProduceXml(false));
                            int bitsize = Convert.ToInt32(xmlDoc.SelectSingleNode("TreeItem/VarDef/VarBitSize").InnerText);
                        ...
                        {
                            item = IoList[IoList[i].PackBitsParrent].item;//item = Inputs.Child[Inputs.ChildCount];
                        }
                        else
                        {
                            TotalOutputs++;
                    ...
                    if (IoList[i].Description != null && IoList[i].Description != "" && IoList[i].PackBitsOffset <= 0)
                    {
                    ...
                    try
                    {
                        //When we need to implement bit packing. HRESULT LinkVariables(BSTRbstrV1, BSTRbstrV2, longoffs1, longoffs2, longsize); 
                        if (IoList[i].PackBitsOffset >= 0 )
                        {
                            if (IoList[i].PathName != "")
                                systemManager.LinkVariables(IoList[i].PathName, item.PathName, 0, IoList[i].PackBitsOffset, 1);
                        }
                        else
                        {
                            if (IoList[i].PathName != "")
        ...
        private void GetIOList_Light(IWorker worker, ITcSmTreeItem EtherCATMaster, List<ReferenceCode.IO_Object> IoList)
        {
            GlobalVariables.IOListNames = new List<string>();
            //GlobalVariables.IOListNames = new List<string>();
            for (int i = 1; i <= EtherCATMaster.ChildCount; i++)
